...
There's a separate version for SteamOS in `sgdb_fetcher_steamos.py`. It only needs the Python that ships with SteamOS and uses your browser as the UI. Run it with:

```
./SteamOS-Launch.sh
```

then open the address it prints (it only listens on 127.0.0.1, nothing is exposed to the network).
